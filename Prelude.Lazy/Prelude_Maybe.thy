...
setup "Header.initialize
       [\"NotFalse\", \"NotTrue\", \"AndFalse\", \"AndTrue\", \"AndSym\",
        ...
        \"IBO11\", \"IBO12\", \"MaybeJustDef\", \"MaybeNothingDef\",
        \"IME01\", \"IME03\", \"IMO01\", \"IMO02\", \"IME02\", \"IMO03\",
        \"IMO04\", \"IMO05\", \"IMO06\", \"IMO07\", \"IMO08\", \"IMO09\",
        \"IMO10\", \"IMO11\", \"IMO12\"]"
...
datatype 'a Maybe = X_Just "'a partial" ("Just/'(_')" [3] 999) |
                    Nothing
...
"ALL (f :: 'a partial => 'b partial).
 ALL (x :: 'a partial).
 ALL (y :: 'b partial). maybe y f (makePartial (Just(x))) = f x"

MaybeNothingDef [rule_format] :
"ALL (f :: 'a partial => 'b partial).
 ALL (y :: 'b partial). maybe y f (makePartial Nothing) = y"

IME01 [rule_format] :
"ALL (x :: 'e partial).
 ALL (y :: 'e partial).
 makePartial (Just(x)) ==' makePartial (Just(y)) = x ==' y"

IME03 [rule_format] :
"ALL (x :: 'e partial).
 ~ makePartial (Just(x)) ==' makePartial Nothing"

IMO01 [rule_format] :
"ALL (x :: 'o partial).
 makePartial Nothing <' makePartial (Just(x))"
...
apply(case_tac "makepartial (Just(x)) <' makePartial Nothing")
apply(auto)
done
...
apply(simp only: GeqDef)
apply(simp only: GeDef OrDef)
apply(case_tac "makePartial Nothing <' makepartial (Just(x))")
apply(auto)
done
...
theorem IMO06 :
"ALL (x :: 'o partial).
 compare (makePartial Nothing) (makePartial (Just(x))) =='
 makePartial EQ =
 makePartial Nothing ==' makePartial (Just(x))"
by (auto)
...
"ALL (x :: 'o partial).
 makePartial Nothing <=' makePartial (Just(x)) =
 X_max (makePartial Nothing) (makePartial (Just(x))) =='
 makePartial (Just(x))"
apply(rule allI)+
...
"ALL (x :: 'o partial).
 makePartial Nothing <=' makePartial (Just(x)) =
 X_min (makePartial Nothing) (makePartial (Just(x))) =='
 makePartial Nothing"
apply(rule allI)+
apply(simp add: LeqDef)
done
