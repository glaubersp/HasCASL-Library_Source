theory Prelude_SortingPrograms_E1
...
ML "Header.initialize
    [\"ga_selector_pre\", \"ga_injective_suc\", \"ga_disjoint_0_suc\",
     ...
     \"ConcatDef\", \"ConcatMapDef\", \"MaximumDef\", \"MinimumDef\",
     \"TakeWhileNil\", \"TakeWhileConsT\", \"TakeWhileConsF\",
     \"DropWhileNil\", \"DropWhileConsT\", \"DropWhileConsF\",
     \"SpanNil\", \"SpanConsT\", \"SpanConsF\", \"SpanThm\",
     \"BreakDef\", \"BreakThm\", \"SplitAtZero\", \"SplitAtNil\",
     \"SplitAt\", \"InsertNil\", \"InsertCons1\", \"InsertCons2\",
     \"DeleteNil\", \"DeleteConsT\", \"DeleteConsF\", \"SelectT\",
     \"SelectF\", \"Partition\", \"PartitionProp\", \"GenSortT1\",
     \"GenSortT2\", \"GenSortF\", \"SplitInsertionSort\",
     \"JoinInsertionSort\", \"InsertionSort\", \"SplitQuickSort\",
     \"JoinQuickSort\", \"QuickSort\", \"SplitSelectionSort\",
     \"JoinSelectionSort\", \"SelectionSort\", \"SplitMergeSort\",
     \"MergeNil\", \"MergeConsNil\", \"MergeConsConsT\",
     \"MergeConsConsF\", \"JoinMergeSort\", \"MergeSort\", \"ElemNil\",
     \"ElemCons\", \"IsOrderedNil\", \"IsOrderedCons\",
     \"IsOrderedConsCons\", \"PermutationNil\", \"PermutationCons\",
     \"PermutationConsCons\", \"Theorem01\", \"Theorem02\",
     \"Theorem03\", \"Theorem04\", \"Theorem05\", \"Theorem06\",
     \"Theorem07\", \"Theorem08\", \"Theorem09\", \"Theorem10\",
     \"Theorem11\", \"Theorem12\", \"Theorem13\", \"Theorem14\"]"
...
"ALL x.
 ...
 (let (ys, zs) = partition (XLtXLtXGt x) xs
...
"ALL xs.
 splitSelectionSort(xs) =
 (let x = minimum(xs) in X_Split x (X_Cons (delete x xs) Nil'))"
...
"ALL X_n.
 ALL xs.
 defOp (length'(xs) div' 2') &
 makePartial X_n = length'(xs) div' 2' -->
 splitMergeSort(xs) =
 (let (ys, zs) = splitAt X_n xs
  in X_Split () (X_Cons ys (X_Cons zs Nil')))"

MergeNil [rule_format] :
"ALL xs. ALL ys. xs = Nil' --> merge xs ys = ys"

MergeConsNil [rule_format] :
"ALL v.
 ALL vs.
 ALL xs. ALL ys. xs = X_Cons v vs & ys = Nil' --> merge xs ys = xs"
...
"ALL v.
 ...
 ALL xs.
 ALL ys.
 (xs = X_Cons v vs & ys = X_Cons w ws) & v <'' w = True' -->
...
"ALL v.
 ...
 ALL xs.
 ALL ys.
 (xs = X_Cons v vs & ys = X_Cons w ws) & v <'' w = False' -->
...
ElemNil [rule_format] : "ALL x. ~ x elem Nil'"

ElemCons [rule_format] :
"ALL x. ALL y. ALL ys. (x elem X_Cons y ys) = (x = y | x elem ys)"

IsOrderedNil [rule_format] : "isOrdered(Nil')"

IsOrderedCons [rule_format] : "ALL x. isOrdered(X_Cons x Nil')"
...
"ALL x.
 ...
 (x <='' y = True' & isOrdered(X_Cons y ys))"

PermutationNil [rule_format] : "permutation(Nil', Nil')"

PermutationCons [rule_format] :
"ALL x. ALL y. permutation(X_Cons x Nil', X_Cons y Nil') = (x = y)"
...
apply(case_tac "b=ac")
apply(simp add: DeleteConsT)
apply(simp add: PermutationConsCons)
apply(simp add: ElemCons)
