...
apply(auto)
apply(induct_tac xs)
apply(simp add: Comp1)
apply(auto)
apply(case_tac "p a")
apply(case_tac "(%a. Not' p a) a")
apply(simp add: FilterConsT FilterConsF)
apply(simp add: NotTrue NotFalse notNot1 notNot2 NotTrue1 NotFalse1)
apply(auto)
thm Partition
apply(simp add: FilterConsT FilterConsF)
apply(simp add: Partition SelectT SelectF)
apply(simp add: NotTrue1 NotFalse1)
apply(auto)
...
setup "Header.record \"PartitionProp\""
...
apply(auto)
apply (simp add: makePartial_def makeTotal_def restrictOp_def)
done

setup "Header.record \"PartialTest\""
