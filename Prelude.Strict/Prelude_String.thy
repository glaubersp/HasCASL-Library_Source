...
setup "Header.initialize
       [\"ga_monotonicity\", \"ga_monotonicity_1\", \"ga_monotonicity_2\",
        ...
        \"ga_comm___XPlus___80\", \"ga_assoc___XPlus___76\",
        \"ga_right_unit___XPlus___90\", \"ga_left_unit___XPlus___88\",
        \"ga_left_comm___XPlus___84\", \"ga_comm___Xx___79\",
        \"ga_assoc___Xx___75\", \"ga_right_unit___Xx___89\",
        \"ga_left_unit___Xx___87\", \"ga_left_comm___Xx___83\",
        \"ga_comm_min_82\", \"ga_comm_max_81\", \"ga_assoc_min_78\",
        \"ga_assoc_max_77\", \"ga_left_comm_min_86\",
...
ICE01 [rule_format] :
"ALL (x :: Char). ALL (y :: Char). ord'(x) ==' ord'(y) = x ==' y"

ICE02 [rule_format] :
"ALL (x :: Char).
 ALL (y :: Char). notH (ord'(x) ==' ord'(y)) = x /= y"

ICO04 [rule_format] :
"ALL (x :: Char). ALL (y :: Char). ord'(x) <_4 ord'(y) = x <_4 y"
...
ICO01 [rule_format] :
"ALL (x :: Char).
 ALL (y :: Char). compare x y ==' EQ = ord'(x) ==' ord'(y)"

ICO02 [rule_format] :
"ALL (x :: Char).
 ALL (y :: Char). compare x y ==' LT = ord'(x) <_4 ord'(y)"

ICO03 [rule_format] :
"ALL (x :: Char).
 ALL (y :: Char). compare x y ==' GT = ord'(x) >_4 ord'(y)"

ICO08 [rule_format] :
"ALL (x :: Char).
 ALL (y :: Char). ord'(x) <=_4 ord'(y) = X_maxX4 x y ==' y"

ICO09 [rule_format] :
"ALL (x :: Char).
 ALL (y :: Char). ord'(y) <=_4 ord'(x) = X_maxX4 x y ==' x"

ICO10 [rule_format] :
"ALL (x :: Char).
 ALL (y :: Char). ord'(x) <=_4 ord'(y) = X_minX4 x y ==' x"

ICO11 [rule_format] :
"ALL (x :: Char).
 ALL (y :: Char). ord'(y) <=_4 ord'(x) = X_minX4 x y ==' y"
...
"ALL (x :: Char).
 ALL (xs :: Char List).
 ALL (y :: Char).
 x ==' y = True' --> X_Cons x xs ==' X_Cons y xs = True'"
apply(auto)
apply(simp add: ILE02)
done

setup "Header.record \"StringT1\""
...
"ALL (x :: Char).
 ...
 ALL (y :: Char).
 ALL (ys :: Char List).
 xs /= ys = True' --> X_Cons x ys ==' X_Cons y xs = False'"
apply(auto)
apply(simp add: ILE02)
apply(case_tac "x ==' y")
apply(auto)
...
setup "Header.record \"StringT2\""

theorem StringT3 :
"ALL (a :: Char List).
 ALL (b :: Char List). a /= b = True' --> a ==' b = False'"
apply(auto)
apply(simp add: DiffDef)
apply(simp add: NotFalse1)
done

setup "Header.record \"StringT3\""

theorem StringT4 :
"ALL (x :: Char).
 ALL (xs :: Char List).
 ALL (y :: Char).
 x <_4 y = True' --> X_Cons x xs <_4 X_Cons y xs = True'"
by (auto)

setup "Header.record \"StringT4\""

theorem StringT5 :
"ALL (x :: Char).
 ALL (y :: Char).
 ALL (z :: Char).
 x <_4 y = True' & y <_4 z = True' -->
 X_Cons x (X_Cons z Nil') <_4 X_Cons x (X_Cons y Nil') = False'"
by (auto)

setup "Header.record \"StringT5\""
