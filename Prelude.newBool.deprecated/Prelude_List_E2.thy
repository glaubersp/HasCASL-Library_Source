...
"ALL e.
 ALL i.
 ALL ts.
 ALL ys. foldl'(i, e, ys ++' ts) = foldl'(i, foldl'(i, e, ys), ts)"
apply(auto)
apply(induct_tac ys)
apply(auto)
...
using assms proof (induct xs arbitrary: ys)
 ...
 then have "ys = Nil'" by (case_tac ys) sorry
 then show "unzip(zip'(Nil', ys)) = (Nil', ys)" by (simp add: ZipNil UnzipNil)
next
 fix x
 fix xs::"'a List"
 fix ys::"'b List"
 ...
 then obtain z zs where ys: "ys = X_Cons z zs" and
   ...
   sorry
 hence H: "unzip(zip'(xs, zs)) = (xs, zs)" using 1 by simp

 have "unzip(zip'(X_Cons x xs, ys)) = unzip(zip'(X_Cons x xs, X_Cons z zs))"
   using ys by simp
 also have "... = unzip(X_Cons (x, z) (zip'(xs, zs)))"
 ...
 finally show "unzip(zip'(X_Cons x xs, ys)) = (X_Cons x xs, ys)" .
qed


apply(auto)
apply(case_tac "(xs,ys)")
apply(case_tac "zip'(xs,ys)")
apply(auto)
apply(simp add: UnzipCons)
apply(simp add: Let_def)
apply(case_tac b)
apply(auto)
apply(case_tac a)
apply(auto)
apply(simp only: ZipConsCons)
apply(auto)

apply(induct_tac "Lista")
apply(induct_tac "Listb")
apply(simp only: ZipConsCons)
apply(simp only: UnzipCons)
apply(auto)
