...
consts Nat2nat :: "X_Nat => nat"
consts nat2Nat :: "nat => X_Nat"
...
lemma diff_induct_Nat : "(!!(x::X_Nat). P x 0') ==> 
  (!!(y::X_Nat). P (0'::X_Nat) (suc'(y))) ==>
  (!!(x ::X_Nat) (y::X_Nat). P x y ==> P (suc'(x)) (suc' (y))) ==> P m n"
...
theorem pre_1 [simp] : "!!x z . x > 0 --> (pre_nat x = makePartial z) =  (makePartial (nat2Nat z) = (pre (nat2Nat x)))"
(*
apply (induct_tac x)
apply (auto)
...
theorem pre_2 [simp] : "!!x z . x >' 0' -->( (pre(x) = makePartial z) = (makePartial (Nat2nat z) = pre_nat (Nat2nat x)))"
(*
apply (induct_tac x)
...
lemma zero_minus_lemma [simp] : "0' -? 0' = makePartial 0'"
...
lemma anything_part_minus_zero [simp] : "! x . x -? 0' = makePartial x"
by (auto simp add: sub_def_Nat)

lemma some_intro [rule_format] : "! x y. makePartial x = makePartial y --> x = y"
sorry
...
lemma minus_question_right_unit : "! x . x -? 0' = makePartial x"
by (auto)
...
lemma Suc_minus_Nat_1 : "!! x (y::X_Nat) . y <=' x --> suc'(x) -! y = suc'(x -! y)"
(*
apply (induct_tac y)
apply (auto)
...
lemma same_pMinus_1 : "!! x . x -? x = makePartial 0'"
by (auto simp add: sub_def_Nat)
...
lemma Suc_minus_Nat_2 : "!! x (y::X_Nat) . x <' y --> suc'(x) -! y = 0'"
(*
apply (induct_tac y)
apply (auto)
...
by (auto simp add: subTotal_def1_Nat)
(*
...
apply (force)
apply (force)
apply (auto simp add: min_def_Nat)
sorry
...
apply (force)
apply (force)
apply (auto simp add: max_def_Nat)
sorry
...
apply (induct_tac y)
apply (auto simp add: X1_def_Nat power_suc_Nat)
sorry
...
lemma divide_zero : "!! x . x ~= 0' --> 0' div' x = makePartial 0'"
apply (auto)
apply (subst div_Nat)
apply (auto)
apply (subst ga_comm___Xx__)
...
lemma mod_less_Nat [simp] : "!! x y . x <' y --> x mod' y = makePartial x"
apply (auto)
apply (subst mod_Nat)
apply (auto)
...
theorem mod_1 [simp] : "!! x y . y ~= 0 --> makePartial (nat2Nat(x mod y)) = nat2Nat(x) mod' nat2Nat(y)"
apply (auto)
apply (rule sym)
apply (subst mod_Nat)
...
theorem div_1 [simp] : "!! x y . y ~= 0 --> makePartial (nat2Nat(x div y)) = nat2Nat(x) div' nat2Nat(y)"
apply (auto)
apply (rule sym)
apply (subst div_Nat)
...
lemma mod_implies_less : "!! x y z . x mod' y = makePartial z --> z <' y"
apply (clarify)
...
theorem mod_2 [simp] : "!! x y z . y ~= 0' --> ((x mod' y = makePatial z) = (Nat2nat(z) = Nat2nat(x) mod Nat2nat(y)))"
(*
apply (auto)
apply (rule sym)
...
theorem div_2 [simp] : "!! x y z . y ~= 0' --> ((x div' y = (makePartial z)) = (Nat2nat(z) = Nat2nat(x) div Nat2nat(y)))"
(*
apply (auto)
apply (rule sym)
apply (drule neq_zero_Nat[rule_format])
...
theorem part_div_2 [simp] : "!! x y z . (0 < y & defOp(x /?? y)) --> (x /?? y = Some z) = ((makePartial (nat2Nat z)) = nat2Nat x /? nat2Nat y)"
apply (auto)
apply (subst (asm) divide_Pos_nat)
apply (force)
apply (rule sym)
...
lemma div_mult_same : "!! x y . y ~= 0' --> ((x *' y) div' y = makePartial x)"
apply (auto simp add: div_Nat)
apply (rule_tac x="0'" in exI)
...
lemma even_two : "!! x . ? y . (x = y *' 2'  --> x div' 2' = makePartial y)"
apply (auto)
...
lemma even_mod [simp] : "!! x . even'(x) = (x mod' 2' = makePartial 0')"
apply (induct_tac x)
apply (subst mod_Nat)
apply (simp)
apply (rule conjI)
...
lemma odd_mod : "!! x . odd'(x) = (x mod' 2' = makePartial 1')"
apply (auto simp add: X1_def_Nat)
apply (auto simp add: X2_def_Nat)
apply (erule lt_mod[rule_format])
sorry
...
apply (auto)
apply (auto simp only: X2_def_Nat)
apply(auto)
sorry
...
apply (rule Nat2nat_inj)
apply (auto)
(*apply (auto simp add: X3_def_Nat)*)
sorry
